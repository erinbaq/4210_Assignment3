...
for num_c in c: #iterates over c
    for num_degree in degree: #iterates over degree
        for num_kernel in kernel: #iterates kernel
           for num_dfs in decision_function_shape: #iterates over decision_function_shape
                ...
                clf = svm.SVC(c= num_c, dergee = num_degree, kernel = num_kernel, decision_function_shape= num_dfs)
                ...
                total = 0
                for samp in dbTest:
                    class_predicted = int(clf.predict([samp[:-1]])[0]
                    if int(class_predicted) == int(samp[-1]):
                        correct += 1
                    total += 1
